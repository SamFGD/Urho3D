...
void PS()
{
    ...
    vec2 noise = (texture2D(sTex2d02, vWaterUV).rg - 0.5) * cNoiseStrength;
    refractUV += noise;
    ...
    vec3 refractColor = texture2D(sTex2d05, refractUV).rgb * cWaterTint;
    vec3 reflectColor = texture2D(sTex2d01, reflectUV).rgb;
