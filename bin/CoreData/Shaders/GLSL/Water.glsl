...
void PS()
{
    ...
    vec2 noise = (texture2D(s02, vWaterUV).rg - 0.5) * cNoiseStrength;
    refractUV += noise;
    ...
    vec3 refractColor = texture2D(s05, refractUV).rgb * cWaterTint;
    vec3 reflectColor = texture2D(s01, reflectUV).rgb;
