...
void PS()
{
    ...
    vec2 noise = (texture2D(sTextureUnit2, vWaterUV).rg - 0.5) * cNoiseStrength;
    refractUV += noise;
    ...
    vec3 refractColor = texture2D(sTextureUnit5, refractUV).rgb * cWaterTint;
    vec3 reflectColor = texture2D(sTextureUnit1, reflectUV).rgb;
