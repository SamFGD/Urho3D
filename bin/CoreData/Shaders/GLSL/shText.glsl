#include "incUniforms.glsl"
#include "incSamplers.glsl"
#include "incTransform.glsl"
...
/*
    ...
    float distance = texture2D(sTextureUnit1, vTexCoord).a;
    if (distance >= 0.5)
...
void PS()
{
    ...
    float distance = texture2D(sTextureUnit1, vTexCoord).a;
    ...
    #ifdef TEXT_EFFECT_SHADOW
        if (texture2D(sTextureUnit1, vTexCoord - cShadowOffset).a > 0.5 && distance <= 0.5)
    ...
    #endif
        {
            ...
            float alpha = GetAlpha(distance, width);
                ...
                float distance2 = texture2D(sTextureUnit1, square.xy).a;
                float distance3 = texture2D(sTextureUnit1, square.zw).a;
                float distance4 = texture2D(sTextureUnit1, square.xw).a;
                float distance5 = texture2D(sTextureUnit1, square.zy).a;
        ...
        gl_FragColor.rgb = vColor.rgb;
        #ifdef GL3
            gl_FragColor.a = vColor.a * texture2D(sTextureUnit1, vTexCoord).r;
        #else
            gl_FragColor.a = vColor.a * texture2D(sTextureUnit1, vTexCoord).a;
        #endif
    #else
        gl_FragColor = vColor * texture2D(sTextureUnit1, vTexCoord);
    #endif
#endif
}
