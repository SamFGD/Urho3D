...
{
    #ifdef BRIGHT
    float3 rgb = Sample2D(Tex2d01, iScreenPos).rgb;
    ...
    float3 rgb = 0.0;
    for (int i = 0; i < 5; ++i)
        rgb += Sample2D(Tex2d01, iTexCoord + (float2(offsets[i], 0.0)) * cBlurHInvSize).rgb * weights[i];
    ...
    float3 rgb = 0.0;
    for (int i = 0; i < 5; ++i)
        rgb += Sample2D(Tex2d01, iTexCoord + (float2(0.0, offsets[i])) * cBlurHInvSize).rgb * weights[i];
    oColor = float4(rgb, 1.0);
    #endif

    #ifdef COMBINE
    float3 original = Sample2D(Tex2d01, iScreenPos).rgb * cBloomMix.x;
    float3 bloom = Sample2D(Tex2d02, iTexCoord).rgb  * cBloomMix.y;
