    ...
    out float4 oColor : OUTCOLOR0)
{
    ...
    float2 noise = (Sample2D(TextureUnit2, iWaterUV).rg - 0.5) * cNoiseStrength;
    refractUV += noise;
    ...
    float3 refractColor = Sample2D(TextureUnit5, refractUV).rgb * cWaterTint;
    float3 reflectColor = Sample2D(TextureUnit1, reflectUV).rgb;
