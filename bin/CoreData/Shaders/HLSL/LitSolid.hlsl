    ...
    out float4 oColor : OUTCOLOR0)
{
    ...
    #ifdef DIFFMAP
        float4 diffInput = Sample2D(TextureUnit1, iTexCoord.xy);
        #ifdef ALPHAMASK
            if (diffInput.a < 0.5)
                discard;
        #endif
    ...
    #ifdef SPECMAP
        float3 specColor = cMatSpecColor.rgb * Sample2D(TextureUnit3, iTexCoord.xy).rgb;
    #else
    ...
    // Get normal
        ...
        float3 normal = normalize(mul(DecodeNormal(Sample2D(TextureUnit2, iTexCoord.xy)), tbn));
    #else
